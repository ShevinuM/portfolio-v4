...
8. ~~**The avatar `<Image>` emitted malformed markup.**~~ **FIXED** in `1a890f9`, outside the phase structure, after a visual check. `src/components/layout/LeftSidebar.astro` line 18 had a `/* ... */` comment inside the `<Image>` attribute list, which Astro parsed as six junk HTML attributes (`*="true" Maximum="true" in="true" global.css="true" for="true" sidebar-avatar="true"`) plus `width="1"`.
